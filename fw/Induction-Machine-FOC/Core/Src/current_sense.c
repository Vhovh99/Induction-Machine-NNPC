...
static CurrentOffset_t offsets = {1.65, 1.65, 1.65, 0};  // Default to mid-scale
...
CurSense_Data_t CurrentSense_Read(void)
{
    ...
    // --------------------------------------------------------------------------
    // ADC1 JDR1 -> Ia (Injected Rank 1 - Channel 1)
    uint32_t raw_Ia = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_1);

    // ADC1 JDR2 -> Ib (Injected Rank 2 - Channel 6)
    uint32_t raw_Ib = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_2);

    // ADC1 JDR3 -> Ic (Injected Rank 3 - Channel 7)
    uint32_t raw_Ic = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_3);

    // ADC1 JDR4 -> Vbus (Injected Rank 4 - Channel 2)
    uint32_t raw_Vbus = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_4);

    data.Ia  = Convert_ADC_To_Current(raw_Ia, offsets.offset_a);
    data.Ib  = Convert_ADC_To_Current(raw_Ib, offsets.offset_b);
    data.Ic  = Convert_ADC_To_Current(raw_Ic, offsets.offset_c);
...
CurSense_Data_t CurrentSense_ReadWithShuntSelection(Phase_t shunt1, Phase_t shunt2)
{
    ...
    // Read Vbus (always available on ADC1 JDR4)
    uint32_t raw_Vbus = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_4);
